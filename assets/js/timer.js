...
function countdown() {
    ...
    let data = target.innerHTML.split("day(s),<br>");
    let d = data.length > 1 ? data : [0, data[0]];
    let time = d[1].split(" : ");
    d = d[0];
    let s = time[2];
    let m = time[1];
    ...
    if (d === 0 && h === 0 && m === 0) {
        if (s % 2 === 0) {
            document.getElementById("editable").style.color = "yellow";
        } else {
...
function setTime(days, hours, mins, secs) {
    ...
    target.innerHTML = "";
    if (days > 0) {
        target.innerHTML += days + "day(s),<br>"
    }
    if (hours >= 0) {
